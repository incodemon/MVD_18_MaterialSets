...
int Parsers::parseOBJ_multi(std::string filename, std::vector<Geometry>& geometries, std::vector<Material>& materials) {
    ...
    if (file.is_open())
    {
        ...
        // DEBUG: artifically create two sets for testing
        //        int total_tris = (int)indices.size()/3;
        //        int one_third = total_tris / 3;
        //        geometries.back().createMaterialSet(one_third, -1);
        //        geometries.back().createMaterialSet(total_tris, -1);
